...
def stargen(x):
	...
	if gg1sma<outerlimit:#can't be outside the system - can happen if starmass>~16.5
		outputstring3+="\n Gas Giant 1\n\t"+str(gg1sma)+" au\n\t"+str((gg1sma**3/starmass)**.5)+" years\n\t"+str(498659569/52594920*math.pi*(starmass/gg1sma)**.5)+" km/s\n\t"+str(round(startemperature*(1-.5)**.25*(2319000/498659569*starradius/2/gg1sma)**.5))+" K"#gas giants have an average albedo closer to .5
	#More gas giants
	ggosma=gg1sma
	ggn=1
	while ggosma*2<outerlimit:
		...
		outputstring3+="\n Gas Giant "+str(ggn)+"\n\t"+str(ggosma)+" au\n\t"+str((ggosma**3/starmass)**.5)+" years\n\t"+str(498659569/52594920*math.pi*(starmass/ggosma)**.5)+" km/s\n\t"+str(round(startemperature*(1-.5)**.25*(2319000/498659569*starradius/2/ggosma)**.5))+" K"
	#Inner worlds!
	rocky=gg1sma
	lastrocky=gg1sma+1
	while rocky/2>innerlimit:
		...
		if 273<startemperature*(1-.3)**.25*(2319000/498659569*starradius/2/rocky)**.5<373 and lastrocky-rocky>.15 and rocky<outerlimit:#can't be outside the system, habitable calc used to be innerhabitable<rocky<outerhabitable
			if (rocky**3/starmass)**.5>1: #use years as unit
				outputstring2+=["\n Habitable\n\t"+str(rocky)+" au\n\t"+str((rocky**3/starmass)**.5)+" years\n\t"+str(498659569/52594920*math.pi*(starmass/rocky)**.5)+" km/s\n\t"+str(round(startemperature*(1-.35)**.25*(2319000/498659569*starradius/2/rocky)**.5))+" K"]#assuming albedo of .3 since like earth yo. i need the weird frac cuz i need to convert solar radii to au
			else: #days
				outputstring2+=["\n Habitable\n\t"+str(rocky)+" au\n\t"+str(365.2425*(rocky**3/starmass)**.5)+" days\n\t"+str(498659569/52594920*math.pi*(starmass/rocky)**.5)+" km/s\n\t"+str(round(startemperature*(1-.35)**.25*(2319000/498659569*starradius/2/rocky)**.5))+" K"]
